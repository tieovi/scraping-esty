...
class ProductsSpider(scrapy.Spider):
    ...
    def __init__(self, search, reviews_option=1, count_max=None, urls_only=False, favorite_min=None, review_min=None, *args, **kwargs):
        if search:
            # Build the search URL
            self.start_urls = ['https://www.etsy.com/search?q={}&explicit=1&order=date_desc&ref=pagination&page=1'.format(search)]
            ...
            if favorite_min:
                self.FAVORITE_MIN = int(favorite_min)

            if review_min:
                self.REVIEW_MIN   = int(review_min)
    ...
    def parse_product(self, response):
        ...
        #LuanVT
        NumberOfReview = response.xpath('//button[@id="same-listing-reviews-tab"]/span/text()').get()
        if NumberOfReview is None:
            print('[LuanVT] No review from customer\n')
        else:
            print('[LuanVT] NumberOfReview: {}\n'.format(int(NumberOfReview.strip())))
        ...
        NumberOfFavorite = response.xpath('//a[contains(text(), " favorites")]/text()').get()
        if NumberOfFavorite is not None:
            NumberOfFavorite = ''.join([c for c in NumberOfFavorite if c in "1234567890"])
            print ('NumberOfFavorite: {}\n'.format(NumberOfFavorite))
        else:
        ...
        # l.add_xpath('number_of_sales', '//div[@class="wt-display-inline-flex-xs wt-align-items-center wt-mb-xs-2 wt-flex-wrap"]//span/text()')

        #@LuanVT
        NumberOfSales = response.xpath('//div[@class="wt-display-inline-flex-xs wt-align-items-center wt-mb-xs-2 wt-flex-wrap"]//span/text()').get()
        if NumberOfSales is not None:
            NumberOfSales = ''.join([c for c in NumberOfSales if c in "1234567890"])

        l.add_xpath('number_of_sales', NumberOfSales)
        # print ('\nNumberOfSales: {}\n'.format(NumberOfSales))

        #@LuanVT detect best seller
        isBestSeller = response.xpath('//div[@id="bestseller-badge-explanation"]/text()').get()
        IsBestSeller = 0
        if isBestSeller is not None:
            print ('Product is BestSeller: {}\n'.format(isBestSeller))
            IsBestSeller = 1
        else:
            print ('Item is not best seller\n')
            IsBestSeller = 0
        ...
        # Option 1
            ...
            #@LuanVT update items counters
            # TODO Update NumberOfSales limit
            ...
            print('REVIEW_MIN: {}'.format(self.REVIEW_MIN))
            print('FAVORITE_MIN: {}'.format(self.FAVORITE_MIN))
            if IsBestSeller:
                if NumberOfReview is not None and int(NumberOfReview) >= self.REVIEW_MIN:
                    if NumberOfFavorite is not None and int(NumberOfFavorite) >= self.FAVORITE_MIN:
                        IncrCounter = 1

            #if NumberOfReview is not None and int(NumberOfReview) >= ReviewLimit:
            if IncrCounter == 1:
                # if int(NumberOfReview) >= ReviewLimit:
                self.COUNTER += 1
                print('\nProducts scraped: {}\n'.format(self.COUNTER))
                yield l.load_item()
